# ドキュメント

このディレクトリは[Diátaxis](https://diataxis.fr/)フレームワークに基づいて構成されています。

## 構造

### [tutorials/](./tutorials/)
初心者向けの学習志向のガイド。ステップバイステップで基本的な使い方を説明。

### [how-to/](./how-to/)
特定のタスクを実行するための実践的なガイド。

- [setup-dev-env.md](./how-to/setup-dev-env.md) - 開発環境のセットアップ
- [customize-zsh.md](./how-to/customize-zsh.md) - Zsh設定のカスタマイズ
- [manage-tmux-sessions.md](./how-to/manage-tmux-sessions.md) - tmuxセッション管理

### [reference/](./reference/)
技術仕様とAPI参照。

- [zsh-modules/](./reference/zsh-modules/) - Zshモジュール仕様
- [commands/](./reference/commands/) - コマンドリファレンス
- [environment-variables.md](./reference/environment-variables.md) - 環境変数一覧

### [explanation/](./explanation/)
概念的な説明と設計の背景。

- [modular-architecture.md](./explanation/modular-architecture.md) - モジュラー設計の理念
- [performance-optimization.md](./explanation/performance-optimization.md) - パフォーマンス最適化戦略

### [standards/](./standards/)
プロジェクトの開発規約。

- [coding.md](./standards/coding.md) - コーディング規約
- [git.md](./standards/git.md) - Git使用規約
- [security.md](./standards/security.md) - セキュリティガイドライン

### [templates/](./templates/)
再利用可能なテンプレート。

## クイックリンク

- 初めての方: [Getting Started](./tutorials/getting-started.md)
- 開発者向け: [開発環境のセットアップ](./how-to/setup-dev-env.md)
- 設計思想: [モジュラーアーキテクチャ](./explanation/modular-architecture.md)