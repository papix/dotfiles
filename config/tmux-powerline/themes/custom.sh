...
TMUX_POWERLINE_DEFAULT_FOREGROUND_COLOR=${TMUX_POWERLINE_DEFAULT_FOREGROUND_COLOR:-'244'}
...
if [ -z "$TMUX_POWERLINE_WINDOW_STATUS_CURRENT" ]; then
    TMUX_POWERLINE_WINDOW_STATUS_CURRENT=(
        "#[fg=colour234,bg=colour31]"
        "$TMUX_POWERLINE_DEFAULT_LEFTSIDE_SEPARATOR"
        "#[fg=colour255,bg=colour31,bold]"
        " #I#F "
        "#[fg=colour31,bg=colour236]"
        "$TMUX_POWERLINE_SEPARATOR_RIGHT_BOLD"
        "#[fg=colour255,bg=colour236,bold]"
        " #W "
...
if [ -z "$TMUX_POWERLINE_WINDOW_STATUS_FORMAT" ]; then
    TMUX_POWERLINE_WINDOW_STATUS_FORMAT=(
        "#[fg=colour234,bg=colour235]"
        "$TMUX_POWERLINE_DEFAULT_LEFTSIDE_SEPARATOR"
        "#{?window_bell_flag,#[#{E:@tp_window_idx_bell_style}],#{?#{||:#{window_activity_flag},#{window_silence_flag}},#[#{E:@tp_window_idx_activity_style}],#[#{E:@tp_window_idx_style}]}}"
        " #I#{?window_flags,#F, } "
        "#[fg=colour235,bg=colour236]"
        "$TMUX_POWERLINE_SEPARATOR_RIGHT_BOLD"
        "#[#{E:@tp_window_name_style}]"
        " #W "
        "#[fg=colour236,bg=colour234]"
        "$TMUX_POWERLINE_DEFAULT_LEFTSIDE_SEPARATOR"
    )
...
if [ -z "$TMUX_POWERLINE_RIGHT_STATUS_SEGMENTS" ]; then
    TMUX_POWERLINE_RIGHT_STATUS_SEGMENTS=(
        ...
        #"xkb_layout 125 117"
        "date_day 235 244"
        "date 235 244 ${TMUX_POWERLINE_SEPARATOR_LEFT_THIN}"
        "time 235 244 ${TMUX_POWERLINE_SEPARATOR_LEFT_THIN}"
        #"utc_time 235 244 ${TMUX_POWERLINE_SEPARATOR_LEFT_THIN}"
    )
fi
