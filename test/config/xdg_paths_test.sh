...
zsh_output="$(env -i HOME="$TMP_HOME" PATH="/usr/bin:/bin" zsh -df -c '
    ...
    printf "DOTFILES_SECRET_MANAGER_VAULT=%s\n" "${DOTFILES_SECRET_MANAGER_VAULT:-}"
    printf "DOTFILES_SECRET_MANAGER_ITEM=%s\n" "${DOTFILES_SECRET_MANAGER_ITEM:-}"
    printf "PATH=%s\n" "$PATH"
...
assert_contains "$zsh_output" 'DOTFILES_SECRET_MANAGER_VAULT=dotfiles' 'zshenv should set default Secret manager vault'
assert_contains "$zsh_output" 'DOTFILES_SECRET_MANAGER_ITEM=shared-env' 'zshenv should set default Secret manager item'
