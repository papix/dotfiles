...
assert_contains 'if [[ -e "${HOME}/.config/gwq/config.toml" && ! -L "${HOME}/.config/gwq/config.toml" ]]; then' "$COMMON_LIB"
assert_contains 'mv "${HOME}/.config/gwq/config.toml" "${HOME}/.config/gwq/config.toml.backup.$(date +%Y%m%d%H%M%S)"' "$COMMON_LIB"
assert_contains 'log_info "gwq config: Backed up existing config"' "$COMMON_LIB"
assert_contains 'mkdir -p "${HOME}/.config/gwq"' "$COMMON_LIB"
assert_contains 'set_config_file "/config/gwq/config.toml" "/.config/gwq/config.toml"' "$COMMON_LIB"
